...
def add_lollipop(
    ...
    y: pd.Series,
    x: pd.Series,
    color: str = COLORS["institutional"],
    highlight_idx: int | None = None,
    highlight_color: str = COLORS["btc"],
) -> None:
    ...
    if highlight_idx is not None:
        ax.vlines(
            x=x_vals[highlight_idx],
            ymin=0,
            ymax=y_vals[highlight_idx],
            color=highlight_color,
            alpha=1.0,
            linewidth=3.5,
        )
        ax.scatter(
            x_vals[highlight_idx], y_vals[highlight_idx], color=highlight_color, s=90, zorder=4
        )
